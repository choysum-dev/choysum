...
func runOneScenario(ctx context.Context, opts RunOptions, packages map[string]*sourceModulePackage, scenario string) error {
	...
	for _, modName := range backendEnvModules {
		...
		for k, v := range modEnv {
			scenarioBackendEnv[k] = v
		}
	}
	extraBackendEnv := formatBackendEnvYAML(scenarioBackendEnv)
...
backendEnv:
	...
	CHOYSUM_E2E_SKIP_INDEX_STALE_SYNC: "true"
%s
task:
	dispatch:
		max_concurrency: 1
...
compile:
  ...
  treeShaking: true
  sourcemap: false
	`, defaultChoysumPath, opts.ModulesPath, distDir, resolvedRuntimeLogLevel, sqliteDSN, authBlock, extraBackendEnv, port)
	...
	// Apply fixtures for modules in the depends closure (each may contribute this scenario).
	uniqueFixtures := uniqueScenarioFixtureModules(closure)
...
type packageScene struct {
	...
	BackendEnv map[string]string `json:"backendEnv"`
}
...
// Callers that need the full run env should merge across the depends closure
// (later modules override earlier ones; the target wins last).
func resolveScenarioBackendEnv(m *sourceModulePackage, scenario string) (map[string]string, error) {
	if m == nil || m.E2E == nil || len(m.E2E.Scenarios) == 0 {
		return nil, nil
	}
	if _, ok := m.E2E.Scenarios[scenario]; !ok {
	...
	seen := map[string]bool{}
	var walk func(name string) (map[string]string, error)
	walk = func(name string) (map[string]string, error) {
		if seen[name] {
			return nil, xfmt.Errorf("scenario extends cycle: %s", name)
		}
		sc, ok := m.E2E.Scenarios[name]
		...
		if parent := strings.TrimSpace(sc.Extends); parent != "" {
			parentEnv, err := walk(parent)
			if err != nil {
				return nil, err
			}
...
// formatBackendEnvYAML renders scenario backendEnv entries as YAML map lines
// (two-space indent, quoted values) with stable key order.
func formatBackendEnvYAML(env map[string]string) string {
	if len(env) == 0 {
		return ""
	}
