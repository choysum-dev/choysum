...
func TestBuildAppBundleFallbackApplicationWithoutPackageJSON(t *testing.T) {
	modulesPath := filepath.Join(t.TempDir(), "modules")
	appDir := filepath.Join(modulesPath, "custom_app")
	...
	if err := buildAppBundle(context.Background(), runtimeScope, nil, "custom_app", "index.js", "custom_app", entry); err != nil {
		t.Fatalf("buildAppBundle() error = %v", err)
	}
	if captured == nil {
		t.Fatal("expected module to be passed to builder hook")
	}
	if captured.ApplicationStr != "custom_app" {
		t.Fatalf("ApplicationStr = %q, want custom_app", captured.ApplicationStr)
	}
}
