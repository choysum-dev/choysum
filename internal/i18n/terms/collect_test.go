...
func TestCollectAllTruncatesWhenTotalExceedsCollected(t *testing.T) {
...
func TestCollectAllLearnsTotalFromSearchPage(t *testing.T) {
	oldPage := ExportPageSize
	ExportPageSize = 1
	t.Cleanup(func() { ExportPageSize = oldPage })
...
func TestCollectAllSearchHookProbeNilTotal(t *testing.T) {
...
func TestCollectAllUsesCountHook(t *testing.T) {
	...
	if countCalls != 1 {
		t.Fatalf("countCalls=%d, want 1", countCalls)
	}
	if searchCalls != 1 {
		t.Fatalf("searchCalls=%d, want 1 page fetch only", searchCalls)
	}
}
