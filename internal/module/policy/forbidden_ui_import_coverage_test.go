...
func TestWebImportSources_PreservesIndentedScriptBody(t *testing.T) {
	...
	srcs, err := webImportSources("Indent.vue", vue)
	if err != nil {
		t.Fatal(err)
	}
	if len(srcs) != 1 {
...
func TestAssertNoForbiddenUiImports_FailsClosedOnUnparsedScript(t *testing.T) {
	modulesPath := t.TempDir()
	webDir := writePartnerWebModule(t, modulesPath, "partner")
	// Close tag followed by non-whitespace text fails the post-</script> guard,
	// so the block is not captured — must not silently skip the file.
	vue := "<template><div /></template>\n<script setup lang=\"ts\">\nimport { DialogRoot } from 'reka-ui';\nexport const x = DialogRoot;\n</script>trailing\n"
	if err := os.WriteFile(filepath.Join(webDir, "BadClose.vue"), []byte(vue), 0o644); err != nil {
		t.Fatal(err)
	}
	err := AssertNoForbiddenUiImports(modulesPath, "partner")
	if err == nil || !strings.Contains(err.Error(), "no script block parsed") {
		t.Fatalf("expected fail-closed unparsed script error, got %v", err)
	}
}
