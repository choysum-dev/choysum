...
func TestGenerateChoyTailwindForModuleWritesFile(t *testing.T) {
	...
	if strings.Contains(body, "*, ::before") || strings.Contains(strings.ToLower(body), "preflight") {
		// Soft check: utilities-only path should not look like a full browser reset.
		t.Logf("note: CSS length=%d (utilities path)", len(body))
	}
