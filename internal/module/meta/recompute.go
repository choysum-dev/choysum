...
// FlushEffective rebuilds effective projections for the given keys (deduped).
// Safe to call inside an outer transaction. This is the sole exported flush entrypoint.
...
func recomputeEffectiveTx(tx *gorm.DB, key LogicalKey) error {
	...
	// Pull differently-named Extends parents (e.g. pkgmeta.BaseModel) into each declaration
...
func deleteEffectiveModelTree(db *gorm.DB, modelID string) error {
	...
	if err := fresh().Model(&pkgmeta.Service{}).Where("model_id = ?", modelID).Pluck("id", &serviceIDs).Error; err != nil {
		return fmt.Errorf("load effective services: %w", err)
	}
	var fieldIDs []string
	if err := fresh().Model(&pkgmeta.Field{}).Where("model_id = ?", modelID).Pluck("id", &fieldIDs).Error; err != nil {
		return fmt.Errorf("load effective fields: %w", err)
	}

	decoratorQ := fresh().Model(&pkgmeta.Decorator{}).Where("model_id = ?", modelID)
	if len(serviceIDs) > 0 {
	...
	if len(decoratorIDs) > 0 {
		if err := deleteWhereFn(fresh(), &pkgmeta.Argument{}, "decorator_id IN ?", decoratorIDs); err != nil {
			return fmt.Errorf("delete effective arguments: %w", err)
		}
		if err := deleteWhereFn(fresh(), &pkgmeta.Decorator{}, "id IN ?", decoratorIDs); err != nil {
			return fmt.Errorf("delete effective decorators: %w", err)
		}
	}
	if len(serviceIDs) > 0 {
		if err := deleteWhereFn(fresh(), &pkgmeta.TypeParameter{}, "service_id IN ?", serviceIDs); err != nil {
			return fmt.Errorf("delete effective type parameters: %w", err)
		}
		if err := deleteWhereFn(fresh(), &pkgmeta.Parameter{}, "service_id IN ?", serviceIDs); err != nil {
			return fmt.Errorf("delete effective parameters: %w", err)
		}
		if err := deleteWhereFn(fresh(), &pkgmeta.Service{}, "id IN ?", serviceIDs); err != nil {
			return fmt.Errorf("delete effective services: %w", err)
		}
	}
	if len(fieldIDs) > 0 {
		if err := deleteWhereFn(fresh(), &pkgmeta.Field{}, "id IN ?", fieldIDs); err != nil {
			return fmt.Errorf("delete effective fields: %w", err)
		}
	}
	if err := deleteWhereFn(fresh(), &pkgmeta.Model{}, "id = ?", modelID); err != nil {
		return fmt.Errorf("delete effective model: %w", err)
	}
	return nil
}

// persistModelTreeAsRaw copies a declaration pkgmeta.Model tree into meta_raw_*.
func persistModelTreeAsRaw(db *gorm.DB, src *pkgmeta.Model) error {
