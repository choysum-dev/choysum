...
// (hangers follow the winning pkgmeta.Field/pkgmeta.Service/pkgmeta.Model — EDS E2 §4.6). Cycles in Extends → error.
//
...
func MergeSameNameModelsByExtensionChain(models []*pkgmeta.Model) (*pkgmeta.Model, error) {
	if len(models) == 0 {
		return nil, nil
	}

	byPath := make(map[string]*pkgmeta.Model, len(models))
	...
	var depthErr error
	var depthOf func(*pkgmeta.Model) int
	depthOf = func(m *pkgmeta.Model) int {
		if m.Path == "" {
			return 0
		}
	...
	mergedFields := make([]*pkgmeta.Field, 0)
	serviceIndex := make(map[string]int)
	mergedServices := make([]*pkgmeta.Service, 0)
...
func mergeEffectiveModel(raws []*rawModel) (*pkgmeta.Model, error) {
	return MergeSameNameModelsByExtensionChain(rawModelsAsModels(raws))
}

// rawModelsAsModels converts raw declaration trees into in-memory pkgmeta.Model trees for E2 merge.
// IDs and timestamps are preserved for ranking; FK targets are not DB-backed.
func rawModelsAsModels(raws []*rawModel) []*pkgmeta.Model {
	out := make([]*pkgmeta.Model, 0, len(raws))
...
func rawModelAsModel(raw *rawModel) *pkgmeta.Model {
	m := &pkgmeta.Model{
...
func rawFieldAsField(f *rawField) *pkgmeta.Field {
	out := &pkgmeta.Field{
...
func rawServiceAsService(s *rawService) *pkgmeta.Service {
	svc := &pkgmeta.Service{
	...
	for _, p := range s.Parameters {
		...
		svc.Parameters = append(svc.Parameters, &pkgmeta.Parameter{
	...
	for _, tp := range s.TypeParameters {
		...
		svc.TypeParameters = append(svc.TypeParameters, &pkgmeta.TypeParameter{
...
func rawDecoratorAsDecorator(d *rawDecorator) *pkgmeta.Decorator {
	out := &pkgmeta.Decorator{
	...
	for _, a := range d.Arguments {
		...
		out.Arguments = append(out.Arguments, &pkgmeta.Argument{
