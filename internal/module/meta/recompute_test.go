...
func TestRecomputeEffective_CreatesPreservesAndDeletes(t *testing.T) {
	...
	base := &rawModel{
		BaseModel:   pkgmeta.BaseModel{Id: sql.NullString{String: "raw-base", Valid: true}, CreatedAt: ts, UpdatedAt: ts},
		Name:        "Partner",
		Application: "partner",
	...
	if err := db.Session(&gorm.Session{SkipHooks: true}).Create(&rawField{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "f-name", Valid: true}, CreatedAt: ts, UpdatedAt: ts},
		Name:      "Name",
		ModelId:   base.Id,
	}).Error; err != nil {
	...
	bank := &rawModel{
		BaseModel:   pkgmeta.BaseModel{Id: sql.NullString{String: "raw-bank", Valid: true}, CreatedAt: ts.Add(time.Hour), UpdatedAt: ts.Add(time.Hour)},
		Name:        "Partner",
		Application: "partner",
	...
	if err := db.Session(&gorm.Session{SkipHooks: true}).Create(&rawField{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "f-bank", Valid: true}, CreatedAt: ts, UpdatedAt: ts},
		Name:      "BankAccounts",
		ModelId:   bank.Id,
	}).Error; err != nil {
	...
	var count int64
	if err := db.Model(&pkgmeta.Model{}).Where("application = ? AND name = ?", "partner", "Partner").Count(&count).Error; err != nil {
...
func TestRecomputeEffective_PreservesServiceIDsByName(t *testing.T) {
	...
	raw := &rawModel{
		BaseModel:   pkgmeta.BaseModel{Id: sql.NullString{String: "raw-i18n", Valid: true}, CreatedAt: ts, UpdatedAt: ts},
		Name:        "I18n",
		Application: "base",
		Path:        "go://i18n/base",
		Abstract:    true,
	...
	for _, name := range []string{"SearchTerms", "UpdateTerm"} {
		if err := db.Session(&gorm.Session{SkipHooks: true}).Create(&rawService{
			BaseModel:             pkgmeta.BaseModel{Id: sql.NullString{String: "rs-" + name, Valid: true}, CreatedAt: ts, UpdatedAt: ts},
			Name:                  name,
...
func TestRecomputeKeys_Dedupes(t *testing.T) {
	...
	var count int64
	if err := db.Model(&pkgmeta.Model{}).Where("name = ?", "X").Count(&count).Error; err != nil {
		t.Fatalf("count: %v", err)
	}
	if count != 1 {
