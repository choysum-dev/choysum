...
func LookupEffectiveModel(db *gorm.DB, application, name string) (*pkgmeta.Model, error) {
	if db == nil {
...
func pickEffectiveAmong(rows []pkgmeta.Model) pkgmeta.Model {
	if len(rows) == 1 {
		return rows[0]
	}
	var best *pkgmeta.Model
	for i := range rows {
		row := &rows[i]
