...
func TestExpandModelsAlongExtends_NilDB(t *testing.T) {
	if err := ExpandModelsAlongExtends(nil, []*pkgmeta.Model{{Name: "X"}}); err == nil || !strings.Contains(err.Error(), "db is nil") {
...
func TestExpandModelsAlongExtends_SkipsNilAndEmptyPath(t *testing.T) {
	...
	child := &pkgmeta.Model{Name: "Child", Path: "/child.ts", Fields: []*pkgmeta.Field{{Name: "A"}}}
	emptyPath := &pkgmeta.Model{Name: "NoPath", Path: "  ", Fields: []*pkgmeta.Field{{Name: "B"}}}
	if err := ExpandModelsAlongExtends(db, []*pkgmeta.Model{nil, child, emptyPath}); err != nil {
...
func TestExpandModelsAlongExtends_CacheHitViaTwoChildren(t *testing.T) {
	...
	parent := &rawModel{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "raw-p", Valid: true}},
		Name:      "Parent",
		Path:      parentPath,
	...
	if err := db.Session(&gorm.Session{SkipHooks: true}).Create(&rawField{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "pf", Valid: true}},
		Name:      "Shared",
		ModelId:   parent.Id,
	}).Error; err != nil {
	...
	if err := db.Session(&gorm.Session{SkipHooks: true}).Create(&rawService{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "ps", Valid: true}},
		Name:      "ParentSvc",
		ModelId:   parent.Id,
	}).Error; err != nil {
	...
	childA := &pkgmeta.Model{Name: "A", Path: "/a.ts", Extends: parentPath, Fields: []*pkgmeta.Field{{Name: "AField"}}}
	childB := &pkgmeta.Model{Name: "B", Path: "/b.ts", Extends: parentPath, Fields: []*pkgmeta.Field{{Name: "BField"}}}
	if err := ExpandModelsAlongExtends(db, []*pkgmeta.Model{childA, childB}); err != nil {
		t.Fatalf("expand: %v", err)
	}
	for _, m := range []*pkgmeta.Model{childA, childB} {
...
func TestExpandModelsAlongExtends_CircularExtends(t *testing.T) {
	...
	a := &pkgmeta.Model{Name: "A", Path: "/a.ts", Extends: "/b.ts"}
	b := &pkgmeta.Model{Name: "B", Path: "/b.ts", Extends: "/a.ts"}
	if err := ExpandModelsAlongExtends(db, []*pkgmeta.Model{a, b}); err == nil || !strings.Contains(err.Error(), "circular extends") {
...
func TestExpandModelsAlongExtends_MissingParentInDB(t *testing.T) {
	...
	child := &pkgmeta.Model{
		...
		Fields:  []*pkgmeta.Field{{Name: "OnlyChild"}},
	}
...
func TestResolveExtendsModel_DBError(t *testing.T) {
	...
	child := &pkgmeta.Model{Name: "Child", Path: "/c.ts", Extends: "/parent.ts", Fields: []*pkgmeta.Field{{Name: "X"}}}
	if err := ExpandModelsAlongExtends(db, []*pkgmeta.Model{child}); err == nil || !strings.Contains(err.Error(), "load raw parent") {
...
func TestExpandModelsAlongExtends_PrefersSameApplicationParent(t *testing.T) {
	...
	if err := db.Session(&gorm.Session{SkipHooks: true}).Create(&rawField{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "ff", Valid: true}},
		Name:      "ForeignOnly",
		ModelId:   foreign.Id,
	}).Error; err != nil {
	...
	if err := db.Session(&gorm.Session{SkipHooks: true}).Create(&rawField{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "hf", Valid: true}},
		Name:      "HomeOnly",
		ModelId:   home.Id,
	}).Error; err != nil {
	...
	child := &pkgmeta.Model{
		...
		Application: "home",
		Extends:     parentPath,
		Fields:      []*pkgmeta.Field{{Name: "ChildField"}},
	}
	if err := ExpandModelsAlongExtends(db, []*pkgmeta.Model{child}); err != nil {
		t.Fatalf("expand: %v", err)
	}
...
func TestExpandModelsAlongExtends_BatchKeepsPerApplicationParentShapes(t *testing.T) {
	...
	if err := db.Session(&gorm.Session{SkipHooks: true}).Create(&rawField{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "batch-ff", Valid: true}},
		Name:      "ForeignOnly",
		ModelId:   foreign.Id,
	}).Error; err != nil {
		t.Fatalf("create foreign field: %v", err)
	...
	if err := db.Session(&gorm.Session{SkipHooks: true}).Create(&rawField{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "batch-hf", Valid: true}},
		Name:      "HomeOnly",
		ModelId:   home.Id,
	}).Error; err != nil {
		t.Fatalf("create home field: %v", err)
	...
	homeChild := &pkgmeta.Model{
		Name:        "HomeChild",
		Path:        "/home/batch-child.ts",
		Application: "home",
		Extends:     parentPath,
		Fields:      []*pkgmeta.Field{{Name: "HomeChildField"}},
	}
	foreignChild := &pkgmeta.Model{
		Name:        "ForeignChild",
		Path:        "/other/batch-child.ts",
		Application: "other",
		Extends:     parentPath,
		Fields:      []*pkgmeta.Field{{Name: "ForeignChildField"}},
	}
	// Expand both in one call so path-keyed caching would cross-contaminate.
	if err := ExpandModelsAlongExtends(db, []*pkgmeta.Model{homeChild, foreignChild}); err != nil {
		t.Fatalf("expand: %v", err)
	}
...
func TestExpandModelsAlongExtends_CrossAppParentFallback(t *testing.T) {
	...
	foreign := &rawModel{
		BaseModel:   pkgmeta.BaseModel{Id: sql.NullString{String: "only-other", Valid: true}},
		Name:        "Base",
		Path:        parentPath,
		Application: "other",
		ModuleId:    sql.NullString{String: "mod-other", Valid: true},
	...
	if err := db.Session(&gorm.Session{SkipHooks: true}).Create(&rawField{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "of", Valid: true}},
		Name:      "OtherField",
		ModelId:   foreign.Id,
	}).Error; err != nil {
...
func TestMergeFieldsForSchema_Coverage(t *testing.T) {
	...
	got, err := mergeFieldsForSchema(
		[]*pkgmeta.Field{nil, {Name: ""}, {Name: "P1"}},
		[]*pkgmeta.Field{nil, {Name: ""}, {Name: "C1"}},
	...
	got, err = mergeFieldsForSchema(
		[]*pkgmeta.Field{{Name: "Dup"}, {Name: "Dup"}},
	...
	got, err = mergeFieldsForSchema(
		[]*pkgmeta.Field{{Name: "F", OriginModelPath: "custom-origin"}},
		nil, parentPath, childPath,
	)
	...
	got, err = mergeFieldsForSchema(
		[]*pkgmeta.Field{{Name: "F", FieldType: "varchar"}},
		[]*pkgmeta.Field{{Name: "F", FieldType: "int"}},
		parentPath, childPath,
	)
	...
	childAdd := &pkgmeta.Field{Name: "Kind"}
	_ = childAdd.SetResolvedSpec(&pkgmeta.FieldResolvedSpec{
		FieldName: "Kind",
	...
	if _, err := mergeFieldsForSchema(nil, []*pkgmeta.Field{childAdd}, parentPath, childPath); err == nil || !strings.Contains(err.Error(), "selectionAdd") {
	...
	childConflict := &pkgmeta.Field{Name: "Status"}
	_ = childConflict.SetResolvedSpec(&pkgmeta.FieldResolvedSpec{
		FieldName: "Status",
		Structural: pkgmeta.FieldStructuralSpec{
			Name:            "Status",
			FieldType:       "selection",
			HasSelectionAdd: true,
			SelectionAdd:    []pkgmeta.FieldSelectionItem{{Value: "x", Label: "X"}},
		},
	})
	if _, err := mergeFieldsForSchema([]*pkgmeta.Field{base}, []*pkgmeta.Field{childConflict}, parentPath, childPath); err == nil || !strings.Contains(err.Error(), "inherited static selection") {
...
func TestMergeServicesForSchema_Coverage(t *testing.T) {
	...
	got := mergeServicesForSchema(
		[]*pkgmeta.Service{nil, {Name: ""}, {Name: "PSvc", OriginModelPath: "kept-origin"}},
		[]*pkgmeta.Service{nil, {Name: ""}, {Name: "CSvc"}},
		parentPath, childPath,
	)
	if len(got) != 2 {
	...
	got = mergeServicesForSchema(
		[]*pkgmeta.Service{{Name: "Dup"}, {Name: "Dup"}},
		nil, parentPath, childPath,
	)
	if len(got) != 1 {
	...
	// child override
	got = mergeServicesForSchema(
		[]*pkgmeta.Service{{Name: "S", TsTypeAnnotation: "old"}},
		[]*pkgmeta.Service{{Name: "S", TsTypeAnnotation: "new"}},
		parentPath, childPath,
	)
...
func TestCloneFieldShallow_Coverage(t *testing.T) {
	...
	src := &pkgmeta.Field{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "f1", Valid: true}, CreatedAt: ts},
		Name:      "Name",
	...
	emptyDec := &pkgmeta.Field{Name: "NoDec", Decorators: []*pkgmeta.Decorator{}}
...
func TestCloneServiceShallow_Coverage(t *testing.T) {
	...
	ts := time.Date(2026, 3, 15, 10, 0, 0, 0, time.UTC)
	src := &pkgmeta.Service{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "s1", Valid: true}, CreatedAt: ts},
		Name:      "Create",
		ModelId:   sql.NullString{String: "mid", Valid: true},
		...
		TypeParameters: []*pkgmeta.TypeParameter{
			nil,
			{Name: "T"},
		},
		Decorators: []*pkgmeta.Decorator{
	...
	empty := &pkgmeta.Service{Name: "Bare"}
...
func TestExpandModelsAlongExtends_LocalParentOnly(t *testing.T) {
	...
	parent := &pkgmeta.Model{
		Name:     "Parent",
		Path:     parentPath,
		Fields:   []*pkgmeta.Field{{Name: "Inherited"}},
		Services: []*pkgmeta.Service{{Name: "InheritedSvc"}},
	}
	child := &pkgmeta.Model{
		Name:    "Child",
		Path:    childPath,
		Extends: parentPath,
		Fields:  []*pkgmeta.Field{{Name: "Own"}},
	}
	if err := ExpandModelsAlongExtends(db, []*pkgmeta.Model{parent, child}); err != nil {
		t.Fatalf("expand: %v", err)
	}
	names := map[string]bool{}
	for _, f := range child.Fields {
...
func TestExpandModelsAlongExtends_MergeFieldsErrorPropagates(t *testing.T) {
	...
	childField := &pkgmeta.Field{Name: "Status"}
	_ = childField.SetResolvedSpec(&pkgmeta.FieldResolvedSpec{
		FieldName: "Status",
		Structural: pkgmeta.FieldStructuralSpec{
			Name:            "Status",
			FieldType:       "selection",
	...
	parent := &pkgmeta.Model{Path: parentPath, Fields: []*pkgmeta.Field{base}}
	child := &pkgmeta.Model{Path: childPath, Extends: parentPath, Fields: []*pkgmeta.Field{childField}}
	if err := ExpandModelsAlongExtends(db, []*pkgmeta.Model{parent, child}); err == nil || !strings.Contains(err.Error(), "inherited static selection") {
...
func TestResolveExtendsModel_FullPreloadFromDB(t *testing.T) {
	...
	parent := &rawModel{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "db-parent", Valid: true}},
		Name:      "DbParent",
		Path:      parentPath,
		ModuleId:  sql.NullString{String: "mod", Valid: true},
	}
	...
	field := &rawField{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "dbf", Valid: true}},
		Name:      "F",
		ModelId:   parent.Id,
	}
	...
	fieldDec := &rawDecorator{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "dbfd", Valid: true}},
		Name:      "FieldDec",
	...
	if err := db.Session(&gorm.Session{SkipHooks: true}).Create(&rawArgument{
		BaseModel:   pkgmeta.BaseModel{Id: sql.NullString{String: "dbfa", Valid: true}},
		Type:        "string",
		Value:       `"x"`,
	...
	svc := &rawService{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "dbs", Valid: true}},
		Name:      "Svc",
		ModelId:   parent.Id,
	}
	...
	svcDec := &rawDecorator{
		BaseModel: pkgmeta.BaseModel{Id: sql.NullString{String: "dbsd", Valid: true}},
		Name:      "SvcDec",
		ServiceId: svc.Id,
	}
	...
	if err := db.Session(&gorm.Session{SkipHooks: true}).Create(&rawArgument{
		BaseModel:   pkgmeta.BaseModel{Id: sql.NullString{String: "dbsa", Valid: true}},
		Type:        "string",
		Value:       `"y"`,
	...
	child := &pkgmeta.Model{Name: "Child", Path: "/db/child.ts", Extends: parentPath}
	if err := ExpandModelsAlongExtends(db, []*pkgmeta.Model{child}); err != nil {
		t.Fatalf("expand: %v", err)
	}
