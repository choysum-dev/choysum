...
func (m *ModuleManager) buildBackendBundlesToDir(ctx context.Context, distBundlesDir string, _ map[string]string) error {
	...
	propertyDefinitionOwners := make([]*meta.Module, 0, len(orderedApps))
	...
	for _, app := range orderedApps {
		...
		if owner := pickPropertyDefinitionOwnerModule(app, mods); owner != nil {
	...
	// re-register C2 FieldDefault / AppSetting / PropertyDefinition / TranslationTerm
	// virtual sources for every non-core app so the final bundles/index.js contains the stores.
...
func ensureBundleC2VirtualImports(builder any, fieldDefaultOwners, appSettingOwners, propertyDefinitionOwners, translationTermOwners []*meta.Module) error {
	...
	owners := make([]*meta.Module, 0, len(fieldDefaultOwners)+len(appSettingOwners)+len(propertyDefinitionOwners)+len(translationTermOwners))
	...
	appendUnique(propertyDefinitionOwners)
...
// pickPropertyDefinitionOwnerModule chooses which module path hosts the C2
// PropertyDefinition virtual file for an application (one store per app). Same
// selection rules as pickFieldDefaultOwnerModule.
func pickPropertyDefinitionOwnerModule(app string, mods []*meta.Module) *meta.Module {
	return pickFieldDefaultOwnerModule(app, mods)
}

// pickTranslationTermOwnerModule chooses the TranslationTerm host module.
// Unlike FieldDefault/AppSetting/PropertyDefinition, empty ServiceEntryPoint is allowed (Ensure).
