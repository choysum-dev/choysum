...
func (m *ModuleManager) generateAppToDirs(ctx context.Context, appName string, modulesTargets pipeline.ModulesAppTargets, distAppDir string) error {
	...
	// Ensure EnsureServiceEntry Specs (TranslationTerm) are in IR before codegen.
	// BundleInject alone puts models into JS without Persist; skipping this leaves
	// getApplication empty for Ensure-only apps (web) and can drop stale protos.
	if err := m.ensureInjectedAppModelsForCodegen(ctx, &mod); err != nil {
		return err
	}
...
// ensureInjectedAppModelsForCodegen rebuilds the app's representative module when
// TranslationTerm is missing from effective IR so GenerateApp can emit protos.
// BundleInject alone does not Persist; Ensure-only apps (web) otherwise stay empty.
func (m *ModuleManager) ensureInjectedAppModelsForCodegen(ctx context.Context, mod *meta.Module) error {
	if m == nil || mod == nil {
		return nil
	}
	...
	var count int64
	if err := m.runtimeScope.Session().Model(&meta.Model{}).
		Where("application = ? AND name = ? AND abstract = ?", app, "TranslationTerm", false).
		Where("(module_id IS NULL OR module_id = '')").
		Count(&count).Error; err != nil {
		return xfmt.Errorf("count TranslationTerm for application %s: %w", app, err)
	}
	if count > 0 {
		return nil
	}
	...
	if err != nil {
		return xfmt.Errorf("rebuild module %s for TranslationTerm inject: %w", mod.Name, err)
	}
...
func (m *ModuleManager) buildBackendAppToDir(ctx context.Context, appName string, distAppDir string) error {
	...
	// Ensure-only apps (web) have no persisted ServiceEntryPoint; still emit a
	// stub entry so BundleInjectAppModels can attach TranslationTerm.
	if !hasEntryImport {
...
func (m *ModuleManager) withLeaseRenewPaused(fn func() error) error {
	...
	started := time.Now()
	err := fn()
	renewInterval := moduleManagerLeaseRenewInterval(moduleManagerLeaseTTL)
	// Lease may already be up to one renew tick old when the pause starts, so the
	// remaining guaranteed window is TTL-tick; warn once elapsed consumes that.
	if depth == 1 && time.Since(started)+renewInterval >= moduleManagerLeaseTTL && m.runtimeScope != nil {
		m.runtimeScope.Logger().Warn("module manager lease renew paused longer than safe window",
			"elapsed", time.Since(started), "renew_interval", renewInterval, "ttl", moduleManagerLeaseTTL)
	}
	return err
}
...
// listInstalledApps returns distinct non-empty application_str values for
// installed modules (including application "web" for TranslationTerm codegen).
func (m *ModuleManager) listInstalledApps(ctx context.Context) ([]string, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	...
	for _, name := range names {
		...
		// Include application "web" so core upgrades regenerate api/web/proto
		// (TranslationTerm EnsureServiceEntry). Global SPA remains dist/web.
