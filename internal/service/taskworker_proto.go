...
func taskWorkerDescriptors(appName string) (protoreflect.MethodDescriptor, protoreflect.MessageDescriptor, protoreflect.MessageDescriptor, protoreflect.MessageDescriptor, error) {
	name := strings.TrimSpace(appName)
	if name == "" {
		name = "taskworker"
	}
	...
	taskWorkerMu.Lock()
	if set, ok := taskWorkerCache[name]; ok {
		taskWorkerMu.Unlock()
		return set.method, set.req, set.resp, set.errMsg, nil
	}
	...
	var errDesc protoreflect.MessageDescriptor
	fields := respDesc.Fields()
	if f := fields.ByName("error"); f != nil {
		errDesc = f.Message()
	}

	set := &taskWorkerDescriptorSet{
		method: methodDesc,
		req:    reqDesc,
		resp:   respDesc,
		errMsg: errDesc,
	...
	taskWorkerMu.Lock()
	taskWorkerCache[name] = set
	delete(taskWorkerErrs, name)
	taskWorkerMu.Unlock()
	return set.method, set.req, set.resp, set.errMsg, nil
}

func taskWorkerStoreError(appName string, err error) (protoreflect.MethodDescriptor, protoreflect.MessageDescriptor, protoreflect.MessageDescriptor, protoreflect.MessageDescriptor, error) {
	taskWorkerMu.Lock()
