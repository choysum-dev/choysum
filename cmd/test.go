...
func newTestUnitCmd(envGetter func() scope.Scope, runtimeOptionsGetter func() cliRuntimeOptions) *cobra.Command {
	...
	cmd.Flags().StringVar(&junitPath, "junit", "", "write JUnit XML report to path")
	...
	cmd.Flags().BoolVar(&coverageReport, "coverage-report", false, "generate coverage report via nyc (requires node + nyc installed)")
