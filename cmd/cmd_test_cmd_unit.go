...
func newTestUnitCmd(envGetter func() scope.Scope, runtimeOptionsGetter func() cliruntime.Options) *cobra.Command {
	...
	cmd.Flags().BoolVar(&withTypecheck, "with-typecheck", false, "run TypeScript typecheck for modules/<app>/service + modules/<app>/web before tests (equivalent to 'choysum test typecheck <app>'; Go-native, no Node)")
	...
	cmd.Flags().BoolVar(&scopeFE, "fe", false, "run frontend tests (QuickJS + choysumtest; Vue host when needed)")
	...
	cmd.Flags().BoolVar(&feCoverageAll, "fe-coverage-all", false, "accepted for CI compatibility; QuickJS FE coverage currently reports executed sources only")
	...
	cmd.Flags().IntVar(&coverageStatements, "coverage-statements", 0, "coverage-check --statements threshold (0 disables)")
