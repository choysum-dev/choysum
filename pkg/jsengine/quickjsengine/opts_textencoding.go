...
func installTextEncodingPolyfill(ctx *quickjs.Context, script string, fileName string) error {
	...
	defer ret.Free()
	if ret.IsException() {
		return fmt.Errorf("failed to install text-encoding polyfill: %w", NormalizeException(ctx.Exception(), "exception without details"))
	}
	return nil
}
...
func toDecodeBytes(v *quickjs.Value) ([]byte, error) {
	...
	if v.IsTypedArray() {
		...
		defer buffer.Free()
		if buffer.IsUndefined() || buffer.IsNull() || !buffer.IsByteArray() {
			return nil, fmt.Errorf("typed array buffer is not an ArrayBuffer")
		}
		...
		if byteOffset.IsUndefined() || byteOffset.IsNull() || byteLength.IsUndefined() || byteLength.IsNull() {
			return nil, fmt.Errorf("typed array bounds are invalid")
		}
