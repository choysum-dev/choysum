  ...
  static async GetEffectiveDeclarations(options?: EffectiveUiResourceDeclarationOptions): Promise<{
    ...
    if (routeIds.length > 0) {
      ...
      const actionIds = Array.from(
        new Set((relationRows || []).map((row: any) => this.readRefId((row as any)?.ActionUiResourceId)).filter((value): value is string => Boolean(value)))
      );
      ...
      for (const row of relationRows || []) {
        const routeId = this.readRefId((row as any)?.RouteUiResourceId);
        const actionId = this.readRefId((row as any)?.ActionUiResourceId);
