...
test('getFieldMetadataView: exposes resolved contract keys when present', () => {
...
test('getFieldMetadataView: marks relation only by declared relation field types', () => {
  const meta: WebFieldMetadata = {
    id: 'f2',
    type: 'ManyToMany',
    typeAnnotation: 'Tag[]',
...
test('getFieldMetadataView: does not infer relation from relationModel alone without relation type', () => {
  const meta: WebFieldMetadata = {
    id: 'f3',
    type: 'String',
