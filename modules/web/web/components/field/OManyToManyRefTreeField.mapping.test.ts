...
test('OManyToManyRefTreeField value mapping > maps tree checked keys to field id array', () => {
  const s = source();
...
test('OManyToManyRefTreeField value mapping > supports children relation loading contract and defaults', () => {
  const s = source();
...
test('OManyToManyRefTreeField value mapping > normalizes model rows and supports toEntity payload', () => {
  const s = source();
...
test('OManyToManyRefTreeField value mapping > supports lazy load and layered preload strategies', () => {
  const s = source();
...
test('OManyToManyRefTreeField value mapping > supports customizable node slots and injects row data', () => {
  const s = source();
...
test('OManyToManyRefTreeField value mapping > keeps node click expand/collapse behavior with custom slots', () => {
  const s = source();
