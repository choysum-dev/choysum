...
test('OManyToManyField mapping/contract > uses many2many generic typing and field registry contract', () => {
  const s = source();
...
test('OManyToManyField mapping/contract > resolves relationStore by binding first then fallback model metadata', () => {
  const s = source();
...
test('OManyToManyField mapping/contract > builds effective conditions by combining exclude-picked, external and onchange', () => {
  const s = source();
...
test('OManyToManyField mapping/contract > maps picker selected records into inserted many2many objects', () => {
  const s = source();
...
test('OManyToManyField mapping/contract > keeps table row key hydration contract for inserted/displayed rows', () => {
  const s = source();
