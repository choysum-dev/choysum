...
const relationStore = computed<WebModelStore<any> | undefined>(() => {
  ...
  try {
    ...
    console.warn(`[OManyToManyRefTreeField] Failed to create store for model '${target}'`, e);
    return undefined;
  }
});
...
const parentQueryFields = computed<any[]>(() => {
  ...
  // Use direct relation field selection so computed OneToMany fields (for example Childs with select.expr)
  // are included even when they don't expose inverseField metadata for nested selection parsing.
...
function applySelectedIds(ids: string[]): void {
  clearItems();
  for (const id of ids) insertItem(id as any);
}
...
async function loadTreeRows() {
  ...
  try {
    ...
    console.warn('[OManyToManyRefTreeField] load tree rows failed', e);
    treeData.value = [];
  } finally {
...
async function loadTreeNode(node: any, resolve: (data: TreeNode[]) => void): Promise<void> {
  ...
  try {
    ...
    console.warn('[OManyToManyRefTreeField] lazy load node children failed', e);
    data.children = [];
