...
test('OManyToManyTagsField mapping/contract > uses object-array many2many typing and field registration contract', () => {
  const s = source();
...
test('OManyToManyTagsField mapping/contract > keeps row key hydration contract for object rows', () => {
  const s = source();
...
test('OManyToManyTagsField mapping/contract > maps picker selected rows into merged object-array values', () => {
  const s = source();
...
test('OManyToManyTagsField mapping/contract > supports keyboard interactions and suggestion highlight', () => {
  const s = source();
...
test('OManyToManyTagsField mapping/contract > wires remote typeahead to NameSearch instead of FE keyword Search', () => {
  const s = source();
...
test('OManyToManyTagsField mapping/contract > wires NameCreate quick-create entry and props', () => {
  const s = source();
...
test('OManyToManyTagsField mapping/contract > keeps selected options in the options list for el-select-v2 tag rendering', () => {
  const s = source();
...
test('OManyToManyTagsField mapping/contract > clears input keyword after selecting an option', () => {
  const s = source();
...
test('OManyToManyTagsField mapping/contract > refreshes search options after removing a tag while dropdown is open', () => {
  const s = source();
...
test('OManyToManyTagsField mapping/contract > supports lightweight tag click mode with auto switch', () => {
  const s = source();
