...
/* ================== Dynamic scale registration (sibling field) ================== */
const scaleFieldName = computed<string | undefined>(() => (binding.meta as any)?.scaleField as string | undefined);

function registerSiblingScaleField() {
  const s = scaleFieldName.value;
  if (!s) return;
  const segs = String(binding.prop).split('.').filter(Boolean);
  if (!segs.length) return;
  segs[segs.length - 1] = s; // Replace the leaf segment with the sibling scaleField
  const scalePath = segs.join('.');
  binding.registerFields(scalePath);
}
onMounted(() => registerSiblingScaleField());
watch([scaleFieldName, () => String(binding.prop)], () => registerSiblingScaleField());

/* ================== Precision / rounding config (field-level) ================== */
const metaPrecision = computed<number | undefined>(() => (binding.meta as any)?.precision);
const metaScale = computed<number | undefined>(() => (binding.meta as any)?.scale);
const effectivePrecision = computed<number>(() => metaPrecision.value ?? props.precision ?? 38);

// Field-level round; fall back to props when metadata is absent
const metaRoundStr = computed<DecimalRound | undefined>(() => (binding.meta as any)?.round as DecimalRound | undefined);
const effectiveRound = computed<Decimal.Rounding>(() => (metaRoundStr.value ? toDecimalRounding(metaRoundStr.value) : props.roundingMode));

/* ================== Helper: resolve dynamic scale ================== */
...
function resolveScaleFrom(obj: any): number {
  try {
    const s = scaleFieldName.value;
    if (s) {
      // Prefer `${scaleField}__max` from the row metrics container in grouped or aggregate scenarios
      const alias = `${s}__max`;
      const viaAgg = obj?.metrics?.[alias];
      const n0 = Number(viaAgg);
      if (Number.isInteger(n0) && n0 >= 0 && n0 <= 18) return n0;

      // Option A: replace the leaf segment in the full binding path for root-record scenarios
      const segs = String(binding.prop).split('.').filter(Boolean);
      if (segs.length) {
        segs[segs.length - 1] = s;
        const viaFullPath = getByPath(obj, segs.join('.'));
        const n1 = Number(viaFullPath);
        if (Number.isInteger(n1) && n1 >= 0 && n1 <= 18) return n1;
      }
      // Option B: sibling field on the row object for list rows and inline editing
      const viaSibling = obj?.[s];
      const n2 = Number(viaSibling);
