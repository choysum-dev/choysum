...
test('formatODecimalDisplayText > pads when a fixed scale is declared', () => {
...
test('formatODecimalDisplayText > keeps significant digits when scale is unset (no zero pad)', () => {
...
test('formatODecimalDisplayText > applies locale separators without forcing extra fractional zeros', () => {
...
test('formatODecimalDisplayText > falls back to plain text when formatFixedDecimalString throws', () => {
  expect(
...
test('resolveODecimalEditScale > uses declared fixed scale, otherwise DB soft max 18', () => {
...
test('resolveODecimalCellScale > prefers a valid getScale result and falls back to 18', () => {
