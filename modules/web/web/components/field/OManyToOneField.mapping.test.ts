...
test('OManyToOneField mapping/contract > uses shared value click payload and emit contract', () => {
  const s = source();
...
test('OManyToOneField mapping/contract > supports auto value click mode by listener detection', () => {
  const s = source();
...
test('OManyToOneField mapping/contract > wires display mode click and keyboard activation', () => {
  const s = source();
...
test('OManyToOneField mapping/contract > wires remote typeahead to NameSearch with DisplayName fields', () => {
  const s = source();
...
test('OManyToOneField mapping/contract > wires NameCreate quick-create entry and props', () => {
  const s = source();
