...
test('resolveUserFilterUserId: treats null, undefined, and empty string as missing', () => {
  expect(resolveUserFilterUserId(null)).toBe('');
  expect(resolveUserFilterUserId(undefined)).toBe('');
  expect(resolveUserFilterUserId('')).toBe('');
});

test('resolveUserFilterUserId: unwraps ManyToOneRef { Id } including null/empty nested Id', () => {
...
test('resolveUserFilterUserId: stringifies bare ids; non-Id objects/arrays count as missing', () => {
...
test('isSharedUserFilterUserId: is true only when resolved owner id is empty', () => {
...
test('userFilterToNamedFilter: maps Condition to query and optional selected', () => {
...
test('userFilterToNamedFilter: trims Name and defaults missing Condition to {}', () => {
...
test('userFilterToNamedFilter: maps falsy Name to empty string', () => {
...
test('isNewerUserFilter: compares UpdatedAt via Date instances and invalid dates as 0', () => {
...
test('isNewerUserFilter: uses CreatedAt when UpdatedAt ties, then Id (including missing Id)', () => {
  const a = {
    Id: 'a',
...
test('pickLatestIsDefault: picks newest private by UpdatedAt then Id', () => {
  const rows = [
...
test('pickLatestIsDefault: falls back to Id when timestamps tie or missing', () => {
  const rows = [
...
test('pickLatestIsDefault: keeps an already-newer first row when later rows are older', () => {
  const rows = [
...
test('pickLatestIsDefault: returns null when no matching IsDefault or rows are nullish', () => {
...
test('mergeUserFilterDefaults: uses shared IsDefault when no private default', () => {
  const merged = mergeUserFilterDefaults({
    privateDefault: null,
...
test('mergeUserFilterDefaults: falls back to code defaults when no server default', () => {
  const merged = mergeUserFilterDefaults({
    privateDefault: null,
    sharedDefault: null,
    codeDefaults: code,
...
test('mergeUserFilterDefaults: ignores private/shared rows that are not IsDefault and accepts a single code default', () => {
  const merged = mergeUserFilterDefaults({
...
test('mergeUserFilterDefaults: drops empty-name code presets when a server default wins', () => {
  const merged = mergeUserFilterDefaults({
...
test('mergeUserFilterDefaults: returns [] when codeDefaults is null/undefined', () => {
  expect(mergeUserFilterDefaults({ codeDefaults: null })).toEqual([]);
  expect(mergeUserFilterDefaults({})).toEqual([]);
});

test('mergeUserFilterDefaults: drops invalid code presets when shared IsDefault wins over non-default private', () => {
  const merged = mergeUserFilterDefaults({
