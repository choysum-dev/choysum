...
test('mergeChatterTimeline: merges messages and field changes by timestamp ascending', () => {
...
test('mergeChatterTimeline: normalizes nullable fields and tie-breaks equal timestamps', () => {
...
test('mergeChatterTimeline: accepts Date and numeric timestamps', () => {
...
test('mergeChatterTimeline: tie-breaks equal timestamps by kind and id', () => {
  const at = '2024-01-01T00:00:00.000Z';
  const entries = mergeChatterTimeline(
    [
...
test('mergeChatterTimeline: normalizes actor and field metadata on field-change rows', () => {
  const entries = mergeChatterTimeline(
    [],
    [
      {
...
test('mergeChatterTimeline: accepts null timelines and defaults blank kinds', () => {
...
test('mergeChatterTimeline: orders same-timestamp field changes before messages in either compare direction', () => {
...
test('mergeChatterTimeline: rejects non-finite numeric timestamps', () => {
