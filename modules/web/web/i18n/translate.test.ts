...
test('createTranslate > uses Composer t against projected Gateway terms', () => {
...
test('createTranslate > falls back to msgid when key missing', () => {
  installI18n();
  const { _t } = createTranslate('web');
  expect(_t('Missing', { scope: 'web/x@y' })).toBe('Missing');
});

test('createTranslate > uses vue-i18n locale fallback without a te preflight', () => {
...
test('createTranslate > uses vue-i18n locale fallback without a te preflight when lang is omitted', () => {
  const reference = createTermReference('base', 'Users', { scope: 'base.route.users' });
  const composer = asTestComposer(createI18n({
    legacy: false,
    locale: 'zh-CN',
...
test('createTranslate > safely handles missing references, composers, and translations', () => {
...
test('createTranslate > reads globalThis.$i18n when window.$i18n is absent', () => {
  const composer = { t: () => 'ok' };
  delete (globalThis as { window?: unknown }).window;
...
test('createTranslate > uses explicit terminology lang when composer locale differs', () => {
  const reference = createTermReference('base', 'Users', { scope: 'base.route.users' });
...
test('createTranslate > reacts to locale changes through the caller composer', () => {
...
test('createTranslate > reacts when the Gateway catalog is merged after the consumer loads', async () => {
  const composer = installI18n();
...
test('createTranslate > uses default and explicit scopes for reactive translations', () => {
  const composer = installI18n();
  composer.mergeLocaleMessage('zh-CN', projectTerminologyMessages({
    auth: {
      'web/pages/Login': {
...
test('createTranslate > separates _t text lookup from _lt term references', () => {
...
test('createTranslate > rejects _lt interpolation at runtime', () => {
...
test('createTranslate > caches factory-default _lt references', () => {
...
test('createTranslate > captures the term reference before computed reevaluation', async () => {
  const composer = installI18n();
  composer.mergeLocaleMessage('zh-CN', projectTerminologyMessages({
    auth: {
      'scope.at.creation': {
...
test('createTranslate > creates deterministic Unicode-safe JSON term references', () => {
...
test('createTranslate > preserves legacy messages while projecting a flat reserved namespace', () => {
  const nested = {
