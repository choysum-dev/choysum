...
test('client service/runtime split: does not bind a lifecycle provider on module import alone', () => {
...
test('client service/runtime split: lazily initializes the default runtime when api state is requested', () => {
  setLifecycleProvider(null);
...
test('client service/runtime split: supports explicit runtime binding and service state scoping', async () => {
  setLifecycleProvider(null);
...
test('client service/runtime split: uses default runtime when no runtime is provided', () => {
  setLifecycleProvider(null);
...
test('client service/runtime split: reads and clears method-scoped loading/error state', () => {
...
test('client service/runtime split: keeps non-function members and merges default context with scoped context', async () => {
  clearContextStack();
  ...
  expect(await api.InspectContext()).toEqual({});

  api.setContext({ tenant: 'T1' });
  expect(await api.InspectContext()).toEqual({ tenant: 'T1' });
