...
test('tracking: tracks nested object field mutations without replacing the whole object', () => {
  const draft = track({
    Preferences: {
      theme: 'light',
...
test('tracking: tracks relation item updates without mutating the original baseline', () => {
...
test('tracking: clears relation dirty state when array is restored to its original shape', () => {
  const draft = track<{ Lines: Array<{ Id?: string; Name: string }> }>({
...
test('tracking: popChanges returns pending updates and resets tracking state', () => {
...
test('tracking: tracks relation create delete and update operations in one payload', () => {
  const draft = track<{ Lines: Array<{ Id?: string; Name: string }> }>({
    Lines: [
...
test('tracking: tracks direct array index assignment and deletion via array proxy traps', () => {
  const draft = track<{ Lines: Array<{ Id?: string; Name: string }> }>({
...
test('tracking: tracks date field updates by value and ignores private proxy fields', () => {
  const draft = track({
...
test('tracking: tracks deleteProperty on nested object proxy and can restore to clean state', () => {
  const draft = track({
