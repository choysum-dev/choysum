...
test('resource declaration helpers: defineRoute maps title to pageTitle and injects resourceId and routeSequence', () => {
  clearResourceDeclarations();
...
test('resource declaration helpers: defineRoute preserves explicit pageTitle override', () => {
  clearResourceDeclarations();
...
test('resource declaration helpers: normalizes term reference titles to English fallbacks plus metadata', () => {
  clearResourceDeclarations();
...
test('resource declaration helpers: defineRoute strips actions from returned route config', () => {
  clearResourceDeclarations();
  const route = defineRoute('auth.route.user_detail', {
    path: '/auth/users/:id',
...
test('resource declaration helpers: defineMenu maps sequence to order and injects resourceId', () => {
  clearResourceDeclarations();
  ...
  expect(getResourceDeclarationFromMeta(menu.meta as any)).toMatchObject({
...
test('resource declaration helpers: defineAction returns stable id and registers explicit action declaration', () => {
  clearResourceDeclarations();
...
test('resource declaration helpers: lists registered declarations for route menu and action uniformly', () => {
  clearResourceDeclarations();
...
test('resource declaration helpers: defineModelActions accepts display config and generates snake ids', () => {
  clearResourceDeclarations();
...
test('resource declaration helpers: defineModelActions registers term reference titles for model actions', () => {
  clearResourceDeclarations();
...
test('resource declaration helpers: defineModelActions returns empty map on invalid model id', () => {
  clearResourceDeclarations();
...
test('resource declaration helpers: throws on invalid resource requires, default roles, and actions', () => {
  clearResourceDeclarations();
  expect(() =>
...
test('resource declaration helpers: accepts nullish list fields and blank/duplicate string entries as empty', () => {
  clearResourceDeclarations();
...
test('resource declaration helpers: dedupes requires and rejects invalid sequence values', () => {
  clearResourceDeclarations();
