...
function storeMeta(ctor: ModelStatic<AppSettingBaseModel>) {
  return MetadataStorage.instance.getModelMetadata(ctor as any);
}

function storeApplication(ctor: ModelStatic<AppSettingBaseModel>): string {
  return String(storeMeta(ctor)?.application || '').trim();
}

/** Empty/`core` stores are invalid. Hard-delete requires softDelete: false. */
function resolveWritableApplication(ctor: ModelStatic<AppSettingBaseModel>): string {
...
async function findByKey(
  ctor: ModelStatic<AppSettingBaseModel>,
  key: string
...
export default class AppSettingBaseModel extends BaseModel {
   ...
   */
  static async Get(
    this: ModelStatic<AppSettingBaseModel>,
    key: string,
