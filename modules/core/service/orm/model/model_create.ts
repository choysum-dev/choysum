...
export class CreateOperations {
  private static stripComputedFields<T extends BaseModel>(ModelCtor: ModelStatic<T>, value: Partial<Insertable<T>>): Partial<Insertable<T>> {
   ...
   */
  static async Create<T extends BaseModel>(ModelCtor: ModelStatic<T>, value: Partial<Insertable<T>>, returnFields?: FieldSelection<T>): Promise<T> {
    const meta = getModelRuntimeMetadata(ModelCtor);
