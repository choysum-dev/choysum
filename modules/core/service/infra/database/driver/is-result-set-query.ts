...
import { DeleteQueryNode, ExplainNode, InsertQueryNode, RawNode, SelectQueryNode, UpdateQueryNode, type CompiledQuery } from "kysely";
...
export function isResultSetQuery(compiledQuery: CompiledQuery): boolean {
  return isSelectQuery(compiledQuery) || isInertQueryWithReturning(compiledQuery) || isExplainQuery(compiledQuery);
}

export function isInertQueryWithReturning(compiledQuery: CompiledQuery): boolean {
  return InsertQueryNode.is(compiledQuery.query) && compiledQuery.query.returning !== undefined;
}
...
function isWithSelectSqlString(sql: string): boolean {
  ...
  const notSelectRegExp = new RegExp(
    [wrapKeyword("delete") + SPACE + "from", wrapKeyword("insert") + SPACE + "into", wrapKeyword("replace") + SPACE + "into", wrapKeyword("update") + SENTENCE + "set"].join(OR),
    "i"
  );
