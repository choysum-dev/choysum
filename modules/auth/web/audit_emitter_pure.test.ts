...
test('createSwitchCompanyScopeAuditEmitter: emitOnce sets emitted state', () => {
...
test('createSwitchCompanyScopeAuditEmitter: emitOnce is idempotent', () => {
  const emitter = createSwitchCompanyScopeAuditEmitter('auth.user.test');
  emitter.emitOnce({ ok: true });
...
test('createSwitchCompanyScopeAuditEmitter: wasEmitted returns false before first emit', () => {
  const emitter = createSwitchCompanyScopeAuditEmitter('auth.user.test');
  expect(emitter.wasEmitted()).toBe(false);
});
