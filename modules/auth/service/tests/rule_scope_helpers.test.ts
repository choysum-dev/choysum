...
test('rule scope helpers: method accepts service/model/application/global shapes', () => {
  const service = { MetaServiceId: 'svc1', MetaModelId: null, MetaApplicationId: null };
  ...
  const model = { MetaServiceId: null, MetaModelId: 'm1', MetaApplicationId: null };
  assertExclusiveScope(model, 'create', 'method');
  expect(model.MetaModelId).toBe('m1');

  const app = { MetaServiceId: null, MetaModelId: null, MetaApplicationId: 'a1' };
  assertExclusiveScope(app, 'create', 'method');
  expect(app.MetaApplicationId).toBe('a1');

  const global = { MetaServiceId: null, MetaModelId: null, MetaApplicationId: null };
...
test('rule scope helpers: field accepts field/model/application/global shapes', () => {
  const field = { MetaFieldId: 'f1', MetaModelId: 'm1', MetaApplicationId: null };
  ...
  const model = { MetaFieldId: null, MetaModelId: 'm1', MetaApplicationId: null };
  assertExclusiveScope(model, 'create', 'field');
  expect(model.MetaModelId).toBe('m1');

  const app = { MetaFieldId: null, MetaModelId: null, MetaApplicationId: 'a1' };
  assertExclusiveScope(app, 'create', 'field');
  expect(app.MetaApplicationId).toBe('a1');

  const global = { MetaFieldId: null, MetaModelId: null, MetaApplicationId: null };
...
test('rule scope helpers: mixed scope throws golden error messages', () => {
  expect(() => assertExclusiveScope({ MetaServiceId: 's1', MetaModelId: 'm1', MetaApplicationId: null }, 'create', 'method')).toThrow(
  ...
  expect(() => assertExclusiveScope({ MetaFieldId: 'f1', MetaModelId: null, MetaApplicationId: null }, 'create', 'field')).toThrow(
...
test('rule scope helpers: update missing sibling fields throws golden messages', () => {
  expect(() => assertExclusiveScope({ MetaModelId: 'm1' }, 'update', 'method')).toThrow(
    'invalid RoleMethodAccess scope update: must provide MetaServiceId/MetaModelId/MetaApplicationId together'
  );
  ...
  expect(() => assertExclusiveScope({ MetaFieldId: 'f1' }, 'update', 'field')).toThrow(
    'invalid RoleFieldRule scope update: must provide MetaFieldId/MetaModelId/MetaApplicationId together'
  );
...
test('rule scope helpers: update with all sibling fields succeeds', () => {
  ...
  const method: Record<string, any> = { MetaServiceId: null, MetaModelId: null, MetaApplicationId: 'a1' };
