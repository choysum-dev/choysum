...
import {
  ...
  documentProbeOwnerRecordForTest} from '../models/_owner_authorization';
import {
  ensureDocumentAuthUserStub,
  disableRepositoryFieldRuleForDocumentTests,
  disableRepositoryRecordRuleForDocumentTests,
  withDocumentAuthUserStubOverride} from './_owner_auth_test_fixtures';
...
async function prepareUploadedSession(ownerRecordId: string, fieldName: string): Promise<{ uploadId: string; businessRequestId: string }> {
  ...
  await UploadSession.UpdateById(
    prepared.uploadId,
    {
      ...
      UploadedPayloadRef: { kind: 'stored_content', storedContentId }} as any,
    ['Id'] as any
  );
  ...
  await withDocumentScope(async () => {
    ...
    await UploadSession.UpdateById(
      prepared.uploadId,
      {
        ...
        UploadedSizeBytes: 1,
        UploadedChecksumSha256: EMPTY_SHA256,
        UploadedContentType: 'text/plain'} as any,
      ['Id'] as any
    );
    try {
...
test('inlined upload coverage: authorize and commit guard rails', async () => {
  ...
  await withDocumentScope(async () => {
    ...
    try {
      await AttachmentObject.AuthorizeUploadPut({ uploadId: uploaded.uploadId});
    ...
    try {
      await AttachmentObject.AuthorizeUploadPut({ uploadId: expiredPrepared.uploadId});
  ...
  await withDocumentScope(async () => {
    await withDocumentAuthUserStubOverride(
      ...
      const withAllowList = await AttachmentObject.AuthorizeUploadPut({
        uploadId: createPrepared.uploadId,
        requestMeta: { contentType: 'image/png', contentLength: 1 }} as any);
      ...
      try {
        await AttachmentObject.AuthorizeUploadPut({
          uploadId: createPrepared.uploadId,
          requestMeta: { contentType: 'text/plain', contentLength: 1 }} as any);
      ...
      await AttachmentObject.AuthorizeUploadPut({
        uploadId: createPrepared.uploadId,
        requestMeta: { contentType: 'image/png' }} as any);
      ...
      await AttachmentObject.CommitUploadPut({
        ...
        payloadReceipt: {
          ...
          sizeBytes: 1,
          checksumSha256: EMPTY_SHA256} as any});
      ...
      await UploadSession.UpdateById(
        finalizePrepared.uploadId,
        {
          ...
          UploadedContentType: null} as any,
        ['Id'] as any
      );
      ...
      await UploadSession.UpdateById(
        emptyMetaPrepared.uploadId,
        {
          ...
          ProposedContentType: null} as any,
        ['Id'] as any
      );
      ...
      try {
        await AttachmentObject.AuthorizeUploadPut({
          uploadId: createPrepared.uploadId,
          requestMeta: { contentLength: 1 }} as any);
