...
When editing `modules/**/package.json` (`choysum.data` / `choysum.demo`) or adding or moving records in `modules/**/data/**/*.json` or `modules/**/demo/**/*.json` (files such as `bootstrap.json`):
...
4. Rule (2) requires the applying module to install **after** auth: it must `depends` on `auth`, and `auth` must **not** depend on it. Modules that auth already depends on (`base`, `meta`) cannot own domain authz init data without a cycle — leave those app-level gift packs in auth (or use a future late-apply mechanism).
